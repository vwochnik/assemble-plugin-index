...
const PLUGIN_NAME = 'assemble-plugin-index';
...
function index(key, options) {
  ...
  if (typeOf(filterFunc) !== 'function') {
    throw new PluginError(PLUGIN_NAME, 'Invalid filter option.');
  }

  if (!template) {
    throw new PluginError(PLUGIN_NAME, 'Index template does not exist.');
  }
